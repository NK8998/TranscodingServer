...
function filterResolutions(inputResolution, specialPresets, presetToChange) {
  ...
  if (presetToChange === "width" || presetToChange === "none") {
    return specialPresets.filter((preset) => preset.height <= inputResolution.height);
  }
  ...
  // filter the presets to include only those with a width less than or equal to the input width
    ...
    return specialPresets.filter((preset) => preset.width <= inputResolution.width);
  }
}
...
function useSpecialPresets(inputResolution, specialPresets, presetToChange) {
  ...
  // Filter the presets
  const filteredResolutions = filterResolutions(inputResolution, specialPresets, presetToChange);
  ...
  const specialResolutions = filteredResolutions.map((preset) => {
    ...
    let newHeight = Math.floor(preset.width * aspectRatio);
